import os
import csv
import re
import h5py
import numpy as np
from pathlib import Path

# ==========================================================
# HDF FILE PATH
# ==========================================================

def get_HDF_for_single_run(run_num, nexus_file):
    return os.path.join(nexus_file, f"VENUS_{run_num}.nxs.h5")

# ==========================================================
# CLEAN ISO TIME
# ==========================================================

def clean_iso(ts):
    if "." not in ts:
        return ts

    main, rest = ts.split(".", 1)

    if "+" in rest:
        frac, tz = rest.split("+", 1)
        tz = "+" + tz
    elif "-" in rest:
        frac, tz = rest.split("-", 1)
        tz = "-" + tz
    else:
        frac, tz = rest, ""

    frac = frac[:6]
    return f"{main}.{frac}{tz}"

# ==========================================================
# FAST RUN SUMMARY
# ==========================================================

def summarize_run(run_num, nexus_file, iso_thresh=0.2):

    hdf5_input = get_HDF_for_single_run(run_num, nexus_file)

    with h5py.File(hdf5_input, "r") as f:

        sample_name = str(
            f["entry/DASlogs/BL10:Exp:IM:ImageFilePath/value"][()]
        ).split("/")[-1]

        T_L = f["entry/DASlogs/BL10:SE:ND2:CH1:PV/value"][()]
        T_R = f["entry/DASlogs/BL10:SE:ND2:CH2:PV/value"][()]

        startT = (float(T_L[0]) + float(T_R[0])) / 2
        endT   = (float(T_L[-1]) + float(T_R[-1])) / 2

        avgT_L = float(f["entry/DASlogs/BL10:SE:ND2:CH1:PV/average_value"][()])
        avgT_R = float(f["entry/DASlogs/BL10:SE:ND2:CH2:PV/average_value"][()])
        avgT = (avgT_L + avgT_R) / 2

        rate_L = float(f["entry/DASlogs/BL10:SE:ND2:Loop1:SP:RateSet/average_value"][()])
        rate_R = float(f["entry/DASlogs/BL10:SE:ND2:Loop2:SP:RateSet/average_value"][()])
        rate = (rate_L + rate_R) / 2

        duration = float(f["entry/duration"][()])

        start_time = clean_iso(
            str(f["entry/start_time"][()].decode()
                if isinstance(f["entry/start_time"][()], bytes)
                else f["entry/start_time"][()])
        )

        end_time = clean_iso(
            str(f["entry/end_time"][()].decode()
                if isinstance(f["entry/end_time"][()], bytes)
                else f["entry/end_time"][()])
        )

    deltaT = endT - startT

    if abs(deltaT) < iso_thresh:
        direction = "Holding"
    elif deltaT > 0:
        direction = "Heating"
    else:
        direction = "Cooling"

    return [
        run_num,
        sample_name,
        avgT,
        direction,
        rate,
        start_time,
        end_time,
        duration,
        startT,
        endT
    ]

# ==========================================================
# RUN NUMBER SCAN
# ==========================================================

def get_run_numbers(master_image_source):

    pattern = re.compile(r"Run_(\d+)")
    run_numbers = []

    for outer in os.scandir(master_image_source):

        if not outer.is_dir():
            continue

        for child in os.scandir(outer.path):

            if not child.is_dir():
                continue

            match = pattern.search(child.name)

            if match:
                run_numbers.append(int(match.group(1)))

    run_numbers = sorted(set(run_numbers))

    print(f"\nTotal runs found: {len(run_numbers)}")
    return run_numbers

# ==========================================================
# MAIN UPDATE METHOD
# ==========================================================

def update_HDF_sheet(output_CSV_path, master_image_source, nexus_file):

    run_nums = get_run_numbers(master_image_source)

    header = [
        "Index",
        "RunNum",
        "SampleName",
        "AvgT",
        "Direction",
        "Rate",
        "StartTime",
        "EndTime",
        "Duration",
        "StartT",
        "EndT"
    ]

    with open(output_CSV_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(header)

        for idx, run_num in enumerate(run_nums):

            try:
                row = summarize_run(run_num, nexus_file)
                writer.writerow([idx] + row)

            except Exception as e:
                print(f"FAILED run {run_num}: {e}")

    print("\nWrote:", output_CSV_path)
    return output_CSV_path