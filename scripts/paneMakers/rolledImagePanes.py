...
def batch_process_images_into_rolls(master_image_source, run_number_range, master_file_destination, scanLen_min, ob_path, roll_length): #will perform a series of data processing steps, and then caclulate plots to make composite image
    ...
    for rollLength in roll_lengths:
        ...
        newImages = process_runs_rolling_global(master_image_source, master_file_destination / (str(rollLength) + "rolls/"), runs_which_have_not_been_processed_yet, ob_path / ("June_OB_" + str(rollLength) + "min.tif" ),  window_size, scanLen_min*60, energyLabels = [560, 830] , duration_percentage_filter = 20)
        tifPaths = []
